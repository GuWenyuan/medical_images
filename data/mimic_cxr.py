...
parser.add_argument(
    '--config', type=str, default='config/config_mimic',
...
class MIMICCXR_DataSource(DataSource):
    ...
    def __init__(self, hparams):
        ...
        np.random.seed(self._hparams["seed"])  # Reset the seed so all runs are the same.
        self.pathologies = sorted(self._hparams["pathologies"])

        self.imgpath = self._hparams["imgpath"]
        self.csvpath = self._hparams["csvpath"]
        self.csv = pd.read_csv(self.csvpath)
        self.metacsvpath = self._hparams["metacsvpath"]
        ...
        self.views = self._hparams["views"]
        self.prepare_csv_entries()
        self.transforms = self.build_transform(self._hparams['transforms']['train'])
